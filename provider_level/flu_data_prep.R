...
colnames(ili_trim) <- c('phys_id','date','state','city','county','zipcode','type_of_practice','total')

flu <- mmwr[,c('mmrw_week_year','pneum_flu')]
colnames(flu) <- c('date', 'deaths')

# aggregate 122 city data to be totals by date
flu_sum <- aggregate(. ~ date,  data = flu, FUN = sum)
...
flu_gold <- flu_sum[which(flu_sum$date >= 200840 & flu_sum$date <= 201239), ]
flu_gold_mat <- matrix(flu_gold[,2])
flu_gold = flu_gold[order(flu_gold$date),]

# reshape ili data so each date has only one row
# make sure not all cells for a provider are NA
ili_trim2 <- ili_trim[,c('phys_id','date','total')]
ili_wide <- reshape(ili_trim2, v.names = 'total', idvar = 'date', timevar = 'phys_id', direction = 'wide')
